...
def json_of_response(rv):
    """Decode json from response"""
    return json.loads(rv.data.decode('utf8'))


def add_message1(client, text):
    """Records a message"""
    rv = client.post('/statuses/update/normaltext', data={'text': text},
                     follow_redirects=True)
    if text:
        assert rv.json == {'Your message was recorded'}
    return rv

def register_and_login1(client, username, password):
    """Registers and logs in in one go"""
    register(client, username, password)
    return login1(client, username, password)
...
def test_login_logout1(client):
    rv = client.get('/account/verify_credentials')
    """Make sure logging in and logging out works"""
    rv = register_and_login1(client, 'user1', 'default')
    assert rv.json == {'You were logged in'}
    rv = logout1(client)
    assert rv.json == {'You were logged out'}
    rv = login1(client, 'user1', 'wrongpassword')
    assert rv.json == {'Invalid password'}
    rv = login1(client, 'user2', 'wrongpassword')
    assert rv.json == {'Invalid username'}


def test_message_recording1(client):
    """Check if adding messages works"""
    register_and_login(client, 'foo', 'default')
    add_message1(client, 'test message 1')
    add_message1(client, '<test message 2>')
    rv = client.post('/statuses/update/foo')
    assert rv.json == {'test message 1'}
    assert rv.json == {'&lt;test message 2&gt;'}


def test_jsontimelines(client):
    """Make sure that timelines(Home, Public & User) work"""
    register_and_login(client, 'foo', 'default')
    add_message(client, 'the message by foo')
    logout(client)
    register_and_login(client, 'bar', 'default')
    add_message(client, 'the message by bar')
    rv = client.get('/statuses/public_timeline')
    assert rv.json == {'the message by foo'}
    assert rv.json == {'the message by bar'}

    # bar's timeline should just show bar's message
    rv = client.get('/statuses/home_timeline')
    '''assert not in rv.json == {'the message by foo'}'''
    assert rv.json == {'the message by bar'}

    # now let's follow foo
    rv = client.get('/foo/follow', follow_redirects=True)
    assert rv.json == {'You are now following &#34;foo&#34;'}
    ...
    # but on the user's page we only want the user's message
    rv = client.get('/statuses/user_timeline/bar')
    '''assert not in rv.json == {'the message by foo'}'''
    assert rv.json == {'the message by bar'}
    rv = client.get('/statuses/user_timeline/foo')
    assert rv.json == {'the message by foo'}
    '''assert not in rv.json == {'the message by bar'}'''

    # now unfollow and check if that worked
    rv = client.get('/foo/unfollow', follow_redirects=True)
    assert rv.json == {'You are no longer following &#34;foo&#34;'}
    rv = client.get('/statuses/home_timeline')
    '''assert not in rv.json == {'the message by foo'}'''
    assert rv.json == {'the message by bar'}


def test_statuses_update(client):
    """Checking if the message has been posted by authenticated user"""
    register_and_login(client, 'foo', 'default')
    add_message(client, 'test message 1')
    rv = client.post('/statuses/update')
    assert rv.json == {'Your message was recorded'}
    return rv
